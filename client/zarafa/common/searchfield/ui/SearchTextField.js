...
Zarafa.common.searchfield.ui.SearchTextField = Ext.extend(Ext.form.TextField, {
	...
	parseQueryToTokens: function(query)
	{
		...
		while ((match = regex.exec(query)) !== null) {
			// A spelling the case-insensitive pattern equates but the alias map
			// does not know stays plain text; a made-up key must never reach the
			// query, the history or the server.
			var key = match[1] ? Zarafa.advancesearch.KQLParser.resolveKeyword(match[1], this.kqlFields) : false;
			if (key) {
				var val = match[2] || '';
				if (val.length >= 2 &&

...
Zarafa.common.ui.htmleditor.WordClipboard = {
	...
	scanGroups: function(rtf)
	{
		...
		for (var i = 0; i < rtf.length; i++) {
			...
			if (c === "\\") {
				...
				if (bin) {
					i += bin[0].length + parseInt(bin[1], 10) - 1;
					continue;
				}
	...
	parsePictures: function(rtf)
	{
		...
		for (var p = 0; p < picts.length; p++) {
			...
			if (nameMatch) {
				// Decode \'xx and \uN escapes, or a non-ASCII name never matches
				// the plain characters the HTML side carries.
				var rawName = nameMatch[1].replace(/\\'([0-9a-fA-F]{2})/g, function(m, h) {
					return String.fromCharCode(parseInt(h, 16));
				});
				rawName = rawName.replace(/\\u(-?\d+)[ ]?\??/g, function(m, n) {
					return String.fromCharCode((parseInt(n, 10) + 65536) % 65536);
				});
				shapeName = rawName.trim();
			}
	...
	findImageSlots: function(html)
	{
		...
		while ((match = re.exec(visible)) !== null) {
			...
			if (/^<v:shape\b[^>]*\/>$/i.test(tag)) {
				continue;                   // child-less, cannot hold a picture
			}
	...
	rewriteImages: function(html, pictures)
	{
		...
		var pick = function(slot) {
			...
			// out of order.
			if (slots.length === pictures.length) {
				for (candidate = 0; candidate < pictures.length; candidate++) {
					if (!used[candidate]) {
						used[candidate] = true;
						return pictures[candidate];

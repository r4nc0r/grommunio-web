...
Zarafa.core.data.UndoManager = Ext.extend(Ext.util.Observable, {
	...
	/**
	 * @cfg {Number} maxTrackedItems Gestures with more records than this are
	 * not captured; the server-side tracking cost scales with the selection.
	 */
	maxTrackedItems: 25,
	...
	/**
	 * Rewrite the recorded ids of history entries referencing the given
	 * items, which got new entryids by being moved.
	 * @param {Object} mapping old entryid -> {entryid, parent_entryid, store_entryid}
	 */
	remapItemIds: function(mapping)
	{
		var apply = function(command) {
			Ext.each(command.items, function(item) {
				var to = mapping[item.ids.entryid];
				if (to) {
					item.ids.entryid = to.entryid;
					item.ids.parent_entryid = to.parent_entryid;
					item.ids.store_entryid = to.store_entryid;
				}
			});
		};
		Ext.each(this.undoStack, apply);
		Ext.each(this.redoStack, apply);
		Ext.each(this.deferredPushes, apply);
	},
	...
	notifyFailure: function(mode, command)
	{
		...
		container.getNotifier().notify('error',
			...
			Ext.util.Format.htmlEncode(String.format(msg, command.description)));
	},
	...
	{
		if (records.length > this.maxTrackedItems) {
			return;
		}
	...
	sweepPending: function()
	{
		...
		var now = Date.now();
		var before = this.pendingItems.length;
		this.pendingItems = this.pendingItems.filter(function(item) {
			return item.expires > now;
		});
		// an expired record may have completed a gesture with buffered items
		if (this.pendingItems.length !== before && this.confirmedBuffer.length > 0) {
			this.scheduleFlush();
		}
	},
	...
	flushConfirmed: function()
	{
		...
		Ext.each(order, function(key) {
			...
			// A ShadowStore gesture answers one response per record; keep
			// the group buffered until none of its records is pending.
			var incomplete = this.pendingItems.some(function(item) {
				return item.gesture.id === group.gesture.id;
			});
			if (incomplete) {
				buffered = buffered.concat(group.items);
				return;
			}
	...
	onStoreException: function(store, proxy, type, action, options, response, arg)
	{
		...
		// a dropped record may have completed a gesture with buffered items
		if (this.confirmedBuffer.length > 0) {
			this.scheduleFlush();
		}
	},
	...
	buildLocationCommand: function(gesture, items)
	{
		...
		Ext.each(items, function(item) {
			...
			// a copy leaves the original in place, older entries keep it
			if (gesture.kind !== 'copy') {
				mapping[item.ids.entryid] = current;
			}
		...
		// keep older entries referencing these items from going stale
		this.remapItemIds(mapping);
	...
	describeGesture: function(kind, items)
	{
		...
		return this.plainText(_('Action'));
	},
	...
	formatSubject: function(subject)
	{
		if (Ext.isEmpty(subject)) {
			return '"' + this.plainText(_('No subject')) + '"';
		}
	...
	formatFolder: function(entryid)
	{
		...
		return this.plainText(_('another folder'));
	},

	/**
	 * Decode a translated text: _() returns html-encoded text, but a
	 * description is plain text which every display site encodes itself.
...
Zarafa.core.data.UndoCommand = Ext.extend(Object, {
	...
	/**
	 * Give a detached record a value for every allowBlank:false field, since
	 * {@link Ext.data.Store#save} silently drops records which fail
	 * {@link Ext.data.Record#isValid} (appointments declare their four date
	 * fields that way). The values go straight into record.data rather than
	 * through set(): non-dirty fields are never written to the server.
	 * @param {Zarafa.core.data.IPMRecord} record The detached record
	 * @private
	 */
	fillRequiredFields: function(record)
	{
		record.undoPlaceholderFields = [];
		record.fields.each(function(field) {
			if (field.allowBlank === false && Ext.isEmpty(record.data[field.name])) {
				record.data[field.name] = this.placeholderFor(field);
				record.undoPlaceholderFields.push(field.name);
			}
		}, this);
	},
	...
	saveDetachedRecords: function(entries, onWritten)
	{
		...
		// The request is serialized above; drop the placeholders again so
		// applyData cannot copy them onto live store records.
		Ext.each(records, function(record) {
			Ext.each(record.undoPlaceholderFields || [], function(name) {
				if (!record.modified || record.modified[name] === undefined) {
					delete record.data[name];
				}
			});
			delete record.undoPlaceholderFields;
		});
	}
});
		...
		this.saveDetachedRecords(entries, function(entries, success) {
			...
			Ext.each(entries, function(entry) {
				...
				if (!Ext.isEmpty(newEntryid)) {
					var to = {
						entryid: newEntryid,
						parent_entryid: entry.target.parent_entryid,
						store_entryid: entry.target.store_entryid
					};
					mapping[entry.item.ids.entryid] = to;
					entry.item.ids.entryid = to.entryid;
					entry.item.ids.parent_entryid = to.parent_entryid;
					entry.item.ids.store_entryid = to.store_entryid;
					viable.push(entry.item);
				}
			});
			this.items = viable;
			// other entries referencing these items must follow the move
			if (this.undoManager) {
				this.undoManager.remapItemIds(mapping);
			}
			callback(true);
		}.createDelegate(this));
	}
	...
	execute: function(mode, callback)
	{
		...
		Ext.each(this.items, function(item) {
			...
			Ext.iterate(values, function(key, value) {
				// force into 'modified': the factory fills every field with
				// its default, and a restored value may equal that default
				record.set(key, value, true);
			});
			record.dirty = true;
			record.endEdit();
